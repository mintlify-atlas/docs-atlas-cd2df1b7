---
title: "Changelog"
description: "All notable changes to Reverse API Engineer, following Semantic Versioning."
---
...
- **`CLAUDECODE` env var leak**: Clear the inherited `CLAUDECODE` env var from CLI subprocesses to prevent nested session interference when running inside Claude Code.
- **CLI stderr noise**: Filter minified JS stack traces into a single clean error line. Use `DEBUG=1` for full output.

**Changed**

- `claude-agent-sdk` minimum version bumped to `0.1.48`.
...
- Background update check now runs in a background thread to avoid blocking CLI startup.
...
- Claude models updated from Claude 4.5 to Claude 4.6 (Opus and Sonnet).
- Sync filtering unified with relative paths; excludes `node_modules`.
- Reduced thinking truncation in TUI for better agent visibility.
...
- **Resource cleanup**: `CopilotClient` is always stopped via `try/finally`.
...
<AccordionGroup>
  <Accordion title="0.3.2 — 2026-01-15">
    ...
    **Changed**
    ...
    - README badges redesigned with flat-square style and centered layout.
    - SVG banner modernized with centered content and improved visual hierarchy.
    - Banner fonts updated for better GitHub rendering support.

    **Fixed**

    - Corrected package name display and font rendering in GitHub banner.
  </Accordion>
  ...
  <Accordion title="0.3.0 — 2026-01-10">
    **Added**
    ...
    - **`@docs` tag**: Generate OpenAPI specifications from HAR files.
      - Standalone: `@docs` generates from the latest run.
      - With run ID: `@docs run_id` generates from a specific run.
    ...
    **Changed**
    ...
    - Improved HAR filtering with better path-based filtering for skip patterns.
    - Fixed pricing computation for OpenCode provider.
    - Refactored latest run parsing logic into centralized run resolution.
  ...
  <Accordion title="0.2.10 — 2026-01-03">
    **Added**
    ...
    - **Example API clients**: Production-ready examples for major platforms.
    ...
    **Changed**
    ...
    - Improved MCP browser integration with better error handling in auto mode.
    - More robust file synchronization with fallback mechanisms.
    - Comprehensive formatting and linting improvements across codebase.

    **Fixed**

    - Reduced unnecessary screenshot captures in agent mode.
    - Fixed sync fallback when primary sync method fails.
    - Corrected `CLAUDE.md` documentation paths.
    - Fixed missing imports in various modules.
  </Accordion>

  <Accordion title="0.2.9 — 2025-12-30">
    **Added**

    - **Real-time file sync**: Watch and automatically sync generated scripts to a local directory.
    ...
    - **MCP browser integration**: Native integration with `rae-playwright-mcp` for auto mode.
      ...
      - Combines browser control and real-time reverse engineering in a single workflow.
    - **`CLAUDE.md` autogeneration**: Automatic generation of project documentation for Claude Code.

    **Changed**

    - Improved settings configuration and UI.
    - Better error handling and resource cleanup for sync operations.

    **Fixed**

    - Fixed issue where sync would overwrite existing directories.
    - Fixed memory leaks when sync errors occurred.
  ...
  <Accordion title="0.2.8 — 2025-12-28">
    **Added**
    ...
    - New pricing entries for Gemini 3 and Claude thinking series models.

    **Changed**

    - Improved prompt handling for code generation in OpenCode.
    - Better folder name generation with OpenCode SDK.
    - Added comprehensive documentation for free models via Antigravity.
  ...
  <Accordion title="0.2.7 — 2025-12-27">
    **Changed**
    ...
    - **Version management**: Implemented single source of truth for versioning.
      ...
      - `__init__.py` reads the version dynamically using `importlib.metadata`.
      - Eliminates the need to update the version in multiple files.
      - Added `RELEASING.md` with release process documentation.
  </Accordion>

  <Accordion title="0.2.6 — 2025-12-27">
    **Fixed**

    - Updated `__version__` to 0.2.6 to ensure `--version` displays correctly.
    - Refactored `OpenCodeEngineer` initialization to properly pop specific kwargs (`opencode_provider` and `opencode_model`) before passing to the parent class.

    **Changed**

    - README: Added table of contents and removed repetitive sections.
  </Accordion>

  <Accordion title="0.2.5 — 2025-12-27">
    **Fixed**

    - Initial release attempt. Superseded by 0.2.6 due to missing version flag update.
  </Accordion>

  <Accordion title="0.2.4 — 2025-12-27">
    **Fixed**

    - `--version` flag now correctly displays 0.2.4 instead of outdated 0.2.0. The previous release (0.2.3) was built with a stale bytecode cache.
    - Added clean build script (`scripts/clean_build.sh`) to prevent future stale builds.
  </Accordion>

  <Accordion title="0.2.3 — 2025-12-27">
    **Changed**

    - Fix hardcoded version display.
    - Remove agent logs and `claude-agent-sdk` logs from output.
    - Better installation instructions for `browser-use` in agent mode.
  </Accordion>

  <Accordion title="0.2.2 — 2025-12-26">
    **Changed**

    - Improved HAR file recording and capture functionality.
  </Accordion>
  ...
  <Accordion title="0.2.1 — 2025-12-26">
    **Added**
    ...
    - **Separate model configurations**: Independent model settings for each SDK and agent provider.
      - `claude_code_model`: Model for Claude SDK (renamed from `model`).
      - `opencode_provider`: Provider for OpenCode SDK.
      - `opencode_model`: Model for OpenCode SDK.
      - `browser_use_model`: Model for the browser-use agent provider.
      - `stagehand_model`: Model for the stagehand agent provider.
    ...
    **Changed**
    ...
    - OpenCode model settings no longer interfere with Claude SDK settings.
    - Automatic migration of old config files to the new structure.
